...
const Navigation = ({ darkModeHandler }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      updateScrolledTop(window.pageYOffset < 10);
    };
  ...
  return (
    <div
      className={`fixed top-0 w-full z-50 duration-300 bg-white dark:bg-black
        ${
          scrolledTop
            ? "bg-white dark:bg-transparent shadow-none"
            : "border-b border-gray-500 dark:border-white/30"
        } ${isMounted ? "translate-y-0" : "-translate-y-20"}`}
    >
      <nav className="relative flex justify-between items-center h-[60px] w-full px-4 md:px-[40px] top-0 dark:text-white">
        {/* Logo and Links */}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {/* Resume*/}
            <a
              className="bg-green-400 rounded-lg px-2 py-1 font-semibold text-black"
              href="/RaazDuraCV.pdf"
